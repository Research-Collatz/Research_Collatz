...
from collatz_graph.node2vec import (
    Node2VecConfig,
...
def test_budget_is_a_hard_limit_without_early_termination() -> None:
    ...
    assert set(graph.edges) == {(1, 4), (2, 1), (4, 2)}
...
def test_bounded_graph_contains_only_in_domain_edges(tmp_path) -> None:
    ...
    assert all(
        target <= 10 and collatz_successor(source) == target
...
def test_graph_statistics_on_small_graph() -> None:
    ...
    assert set(result.spectral["statistic"]) >= {
        "adjacency_spectral_radius_estimate",
        "adjacency_frobenius_norm",
    }
