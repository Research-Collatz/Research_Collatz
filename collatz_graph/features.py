...
def _arithmetic_features(n: int, *, show_progress: bool) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    for start in tqdm(range(1, n + 1), desc="Computing arithmetic features", disable=not show_progress):
        value = start
        steps = 0
        maximum = start
        ...
        while True:
            ...
            if first_lower >= 0 and value > n and value in external_cache:
                ...
                break
            if steps >= 1_000_000:
                raise RuntimeError(f"trajectory for {start} exceeded the step limit")
        ...
        for path_value, path_steps in reversed(path):
            known_total += 1
    ...
    return stopping[1:], total[1:], excursion[1:], parity_length[1:]
...
def compute_node_features(
    ...
    *,
    ...
    1. ``maximum_excursion`` is the largest value visited, including ``n``.
    ...
    stopping, total, excursion, parity_length = _arithmetic_features(n, show_progress=show_progress)
