    ...
    with plt.rc_context(PLOT_STYLE):
        ...
        if np.any(~valid):
            ax.scatter(
                ...
                linewidths=0,
                rasterized=True,
                label="Outside finite root component",
            )
        ...
        if categorical:
            arguments["norm"] = BoundaryNorm(
                np.arange(-0.5, int(np.nanmax(colour_values)) + 1.5),
                plt.get_cmap("viridis").N,
            )
...
def create_umap_figures(
    ...
    ).to_csv(output / "umap_features.csv", index=False)
    specifications = {
        "stopping_time": ("stopping time", False, False),
        "maximum_excursion": ("maximum excursion", True, False),
        "binary_length": ("binary length", False, True),
        "distance_from_root": ("distance from root", False, True),
        "level_set": ("inverse-tree level set", False, True),
    }
    paths: dict[str, Path] = {
        "coordinates": output / "umap_coordinates.npy",
        "features": output / "umap_features.csv",
    }
